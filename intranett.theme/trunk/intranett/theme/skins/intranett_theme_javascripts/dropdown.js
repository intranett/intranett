...
function toggleMenuHandler(event) {
    var menu, actionContent;
    // swap between activated and deactivated
    if (jQuery('dl.actionMenu.activated').length > 0) {
        hideAllMenus(0);
    }

    menu = jQuery(event.target).parents('.actionMenu:first')[0];
    actionContent = jQuery(menu).find('dd.actionMenuContent')[0];
...
function actionMenuMouseOver(event) {
    var switch_menu, menu, actionContent;

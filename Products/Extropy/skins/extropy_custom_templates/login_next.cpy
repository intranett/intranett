...
if came_from is not None:
    ...
    if template_id in ['login', 'login_success', 'login_password', 'login_failed',
                       ...
                       'localhost'] \
        or path in ['/front-page/document_view']:
        came_from = ''
...
if not came_from:
    ...
    if getattr(member, 'UID', None) is not None:
        member = context.reference_catalog.lookupObject(member.UID())
    came_from = member.restrictedTraverse('@@logindestination')()
